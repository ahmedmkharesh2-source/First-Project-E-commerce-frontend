...
const ViewAllUsers = () => {
  ...
  const getAllUsers = async () => {
    try {
      const response = await axios.get("https://first-project-e-commerce-backend-production.up.railway.app/api/v1/users/get-all-users", { withCredentials: true });
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    try {
      const response = await axios.put(
        `https://first-project-e-commerce-backend-production.up.railway.app/api/v1/users/change-user-role/${userId}`,
        { role: newRole },
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      await axios.delete(
        `https://first-project-e-commerce-backend-production.up.railway.app/api/v1/users/delete-user/${userId}`,
        { withCredentials: true }
      );
